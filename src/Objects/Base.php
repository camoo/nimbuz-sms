...
class Base
{
    ...
    public function set(string $sProperty, $value, $oClass = null)
    {
        ...
        if (!property_exists($oClass, $sProperty)) {
            throw new NimbuzSmsException([$sProperty => 'is not allowed!']);
        }
        if ($sProperty === 'from') {
    ...
    public function get($oClass, $validator = 'default')
    {
        ...
        if (method_exists($oClass, 'validator' .ucfirst($validator))) {
            ...
            if ($oValidator->validate() === false) {
                throw new NimbuzSmsException($oValidator->errors());
            }
        }
        if (array_key_exists('route', $hPayload) && $hPayload['route'] === 'classic' && $oClass instanceof \Nimbuz\Sms\Objects\Message && array_key_exists('to', $hPayload)) {
            $asTo = $hPayload['to'];
            foreach ($asTo as $xTo) {
                $sTo = is_array($xTo) && !empty($xTo['mobile'])? $xTo['mobile'] : $xTo;
                if (!Utils::isValidPhoneNumber($sTo, 'CM', true)) {
                    throw new NimbuzSmsException([$sTo => 'does not seems to be a cameroonian phone number!']);
                }
            }
        }
